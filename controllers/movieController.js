...
exports.movie_list = asyncHandler(async (req, res, next) => {
  const allBooks = await Book.find({}, "title director")
    .sort({ title: 1 })
    .populate("director")
    .exec();

  res.render("book_list", { title: "Movie List", movie_list: allBooks });
});
...
exports.movie_detail = asyncHandler(async (req, res, next) => {
  ...
  const [book] = await Promise.all([
    Book.findById(req.params.id).populate("director").populate("genre").exec(),
  ]);
